#!/usr/bin/env bash
#
# mayhem/test.sh — RUN nostril's upstream test suite (pytest form, mayhem/test_upstream_suite.py —
# the same known-answer asserts + dataset evaluations as upstream's tests/test.py script, with its
# printed expectations turned into hard assertions) and emit a CTRF (ctrf.io) summary. exit 0 iff
# failed==0. PATCH-grade oracle: a no-op patch that neuters nonsense() breaks the known-answer and
# dataset-accuracy assertions, so it FAILS here (anti-reward-hacking).
#
# It does NOT compile — build.sh installed pytest + deps into the in-image site dir and compiled
# the nostril_run_tests ELF wrapper. The suite is routed through that compiled NON-system wrapper
...
# Put the in-image site dir (atheris + pytest + nostril deps) on PYTHONPATH.
...
# Run the suite. -p no:cacheprovider keeps the read-only image happy.
LOG="$(mktemp)"
"$RUNNER" -p no:cacheprovider -o addopts= -q "$SRC/mayhem/test_upstream_suite.py" 2>&1 | tee "$LOG"
rc=${PIPESTATUS[0]}

# Parse pytest's summary line, e.g. "17 passed, 1 skipped in 20.4s".
